...
def check_git_status():
    ...
    try:
        ...
        remote_check = subprocess.check_output(
            ["git", "status", "-sb"], text=True
        ).strip()
        if "behind" in remote_check:
            print(
                f"⚠️ Tu rama local esta por detrás de origin/{branch}. Ejecuta: git pull origin {branch}"
            )
            return False
        elif "ahead" in remote_check:
            print(
                f"⚠️ Tienes commits locales sin subir a GitHub. Ejecuta: git push origin {branch}"
            )
            return False
        else:
            print(
                f"✅ Tu repositorio local está 100% al día con origin/{branch} (PavelBT/Applio)."
            )
            return True
...
def main():
    ...
    if git_ok and syntax_ok:
        ...
        print(
            "   Puedes abrir Colab e iniciar entrenamiento con la garantía de tener la última versión."
        )
    else:
        print(
            "⚠️ Revisa los avisos anteriores antes de iniciar tus celdas en Google Colab."
        )
